...
```javascript
const updateMerchant = async (id, pinduoduoShopId, pinduoduoName) => {
  try {
...
def update_merchant(id, pinduoduo_shop_id=None, pinduoduo_name=None):
...
4. **商家ID验证**: `id` 参数是商家系统ID（数据库主键），必须提供有效的商家ID，否则返回 404 错误
